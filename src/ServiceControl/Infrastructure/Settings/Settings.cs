namespace ServiceBus.Management.Infrastructure.Settings {
	...
	public static class Settings {
		static Settings() {
			...
			ElasticSearchHostName = GetElasticSearchHostName();
			ElasticSearchPort = GetElasticSearchPort();
			ElasticSearchIndex = GetElasticSearchIndex();
			ElasticSearchStorageEnabled = GetElasticStorageEnabled();
		}

		public static string ElasticSearchPort;
		public static string ElasticSearchHostName;
		public static string ElasticSearchIndex;
		public static bool ElasticSearchStorageEnabled;

		static bool GetElasticStorageEnabled() {
			var value = SettingsReader<bool>.Read("ElasticSearch", "StorageEnabled", false);

			if (value == false) {
				Logger.Info("Elastic search storage has been disabled because either the config setting 'ElasticSearch/StorageEnabled was false or the config does not exist");
				return false;
			}
			return value;
		}

		static string GetElasticSearchHostName() {
			var value = SettingsReader<string>.Read("ElasticSearch", "HostName", "localhost");

			if (value == null) {
				Logger.Info("The following key: ElastSearch/HostName was not found in configuration. Using 'localhost' as default");
				return "localhost";
			}
			return value;
		}

		static string GetElasticSearchPort() {
			var value = SettingsReader<string>.Read("ElasticSearch", "Port", "9200");

			if (value == null) {
				Logger.Info("The following key: ElastSearch/Port was not found in configuration. Using '9200' as default");
				return "9200";
			}
			return value;
		}
		static string GetElasticSearchIndex() {
			var value = SettingsReader<string>.Read("ElasticSearch", "Index", "allmessages");

			if (value == null) {
				Logger.Info("The following key: ElastSearch/Port was not found in configuration. Using 'allmessages' as default");
				return "allmessages";
			}
			return value;
		}
